...
if uploaded_file is not None:
    ...
    if image_raw is not None:
        st.image(image_raw, caption="Uploaded image", use_column_width=True, channels="GRAY")
    else:
...
image_center = cv2.circle(image_center, (x, y), radius_pupil, (255, 0, 0), 20)
image_center = cv2.circle(image_center, (x, y), 0, (255, 0, 0), 10)
...
# zaznaczenie środka i promienia
image_center = cv2.cvtColor(image_raw.copy(), cv2.COLOR_GRAY2BGR)
image_center = cv2.circle(image_center, (x, y), radius_iris, (255, 0, 0), 20)
...
normalized = norm 
st.image(normalized, caption='Normalized Iris', channels="GRAY")
normalized = normalize_iris(image, x, y, radius_pupil, radius_iris)


code = iris_code(normalized, f=3)
st.image(code * 255, caption='Iris Code', channels="GRAY")